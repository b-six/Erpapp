...
$("#id_pelanggan").change(function() {
	...
	$("#show-wilayah").attr("value", wilayah);
	$("#show-tipe-customer").attr("value", tipe_customer);
});
...
$("#id_pelanggan-edit").change(function() {
	...
	var wilayah = $("#" + id_pelanggan).val();
	var tipe_customer = $("#x" + id_pelanggan).val();
	$("#show-wilayah-edit").attr("value", wilayah);
	$("#show-tipe-customer-edit").attr("value", tipe_customer);
});
...
function getIdEdit(clicked_id) {
	...
	$("#id_so_edit").css("font-weight", "bold");
...
function getIdDelete(clicked_id) {
	...
	$("#id_so_delete").attr("value", id_delete);
}
...
function getIdEdit_SC(id_sc) {
	...
	$("#jumlah_barang_now").attr("value", jumlah_barang_now);
	$("#total_harga-before").attr("value", total_harga);
	$("#id_so-edit").attr("value", id_so_edit);
}
...
$("#id_produk-edit").change(function() {
	...
	var harga_satuan = $("#" + id_produk).val();
	$("#harga_satuan-edit").attr("value", harga_satuan);
});

$("jumlah_barang-edit").keypress(function() {
	$("#total_harga-edit").val(
		$("#jumlah_barang-edit").val() * $("#" + id_produk).val()
	);
});
...
function getIdDelete_PO(clicked_id) {
	var id_delete = clicked_id;
	$("#id_po_delete").attr("value", id_delete);
}
...
function deleteSC(clicked_id) {
	...
	$("#id_sc_delete").attr("value", id_sc);
	var x = $("#" + id_sc).data("id_so_edit");
	$("#id_so_delete").attr("value", x);
	$("#jumlah_barang_so").attr("value", $("#" + id_sc).data("jumlah_barang_so"));
	$("#jumlah_barang_item").attr(
		"value",
		$("#" + id_sc).data("jumlah_barang_now")
	);
	$("#harga_item").attr("value", $("#" + id_sc).data("total_harga"));
	$("#harga_so").attr("value", $("#" + id_sc).data("harga_so"));
}
...
function getIdEdit_cust(clicked_id) {
	...
	$("#nama_pelanggan-edit").attr(
		"value",
		$("#id_pelanggan_" + id_pelanggan).data("nama_cust")
	);
	$("#wilayah-edit").attr(
		"value",
		$("#id_pelanggan_" + id_pelanggan).data("wilayah_cust")
	);
}
...
function getId_cust(clicked_id) {
	...
	$("#id_cust_delete").attr("value", id_pelanggan);
}
...
function getIdEdit_test(test_edit_id) {
	...
	id_so = y[2];
	$("#id_so-edit").attr("value", id_so);
	$("#show-nama-edit").attr("value", $("#get-nama-" + id_so).val());
	$("#show-wilayah-edit").attr("value", $("#get-wilayah-" + id_so).val());
	$("#show-tipe-customer-edit").attr("value", $("#get-tipe-" + id_so).val());
	$("#id_test-edit").attr("value", $("#" + test_edit_id).data("id_test"));
}
...
function getIdDelete_test(test_delete_id) {
	...
	id_testimoni = z[1];
	$("#id_test_delete").attr("value", id_testimoni);
	$("#id_so_delete_test").attr("value", $("#" + test_delete_id).data("id_so"));
}
